...
@section('content')
    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            <a href="{{ route('nongkrong.show', $session) }}" class="text-sm font-semibold text-indigo-600 hover:underline">← {{ $session->name }}</a>
            <h1 class="mt-1 text-2xl font-extrabold text-slate-900">Utang-Piutang</h1>
                        ...
                        <li class="flex items-center justify-between gap-2 py-3">
                            <div class="min-w-0">
                                <p class="truncate text-sm text-slate-700">
                                    ...
                                    <span class="text-slate-400">ke</span>
                                    <strong>{{ $debt->creditor->name }}</strong>
                                </p>
                                <p class="text-xs text-slate-400">
                                    Rp{{ number_format($debt->outstanding(), 0, ',', '.') }}
                                    @if ($debt->paid_amount > 0)
                                        <span class="text-emerald-500">· kebayar Rp{{ number_format($debt->paid_amount, 0, ',', '.') }}</span>
                                    @endif
                            ...
                            <div class="flex shrink-0 items-center gap-2">
                                ...
                                <a href="{{ route('debts.show', [$session, $debt]) }}" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-indigo-600 hover:bg-indigo-50">Detail</a>
                            </div>
                        </li>
                    @endforeach
                </ul>
